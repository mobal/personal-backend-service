...
parser.add_argument(
    '-c', '--coverage-file', required=True, type=str, help='path of the coverage file'
)
parser.add_argument(
    '-f', '--find', required=True, type=str, help='path to find for replace'
)
...
def _replace_source(et: ElementTree, find: str, replace: str) -> ElementTree:
...
def main(coverage_file: str, path_to_find: str, path_to_replace: str):
    ...
    logger.info(f'Find and replace source {path_to_find=} to {path_to_replace=}')
