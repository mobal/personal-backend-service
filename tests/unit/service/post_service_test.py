...
class TestPostService:
    ...
    async def test_successfully_delete_post(self, post_service, post, table):
        await post_service.delete_post(post['id'])
        response = table.query(
            KeyConditionExpression=Key('id').eq(post['id']),
            FilterExpression=Attr('deleted_at').eq(None)
        )
        assert response['Count'] == 0

    async def test_successfully_get_all_posts(self, post_service, post) -> None:
        result = await post_service.get_all_posts()
        assert len(result) == 1
        assert post == result[0].dict()

    async def test_successfully_get_post_by_uuid(self, post_service, post) -> None:
        result = await post_service.get_post(post['id'])
        assert post == result.dict()

    async def test_fail_to_get_post_by_uuid(self, post_service) -> None:
        result = await post_service.get_post(str(uuid.uuid4()))
        assert result is None

    async def test_successfully_update_post(self, post_service, post, table) -> None:
        await post_service.update_post(post['id'], {'content': 'Updated content'})
        response = table.query(
            KeyConditionExpression=Key('id').eq(post['id']),
            FilterExpression=Attr('deleted_at').eq(None)
        )
