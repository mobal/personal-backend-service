...
@pytest.mark.asyncio
class TestJWTAuth:
    async def test_fail_to_authorize_request_due_to_authorization_header_is_empty(self, empty_request, jwt_auth):
    ...
    async def test_fail_to_authorize_request_due_to_authorization_header_is_missing(self, empty_request, jwt_auth):
        with pytest.raises(HTTPException) as excinfo:
            await jwt_auth(empty_request)
        assert NOT_AUTHENTICATED == excinfo.value.detail
        assert status.HTTP_403_FORBIDDEN == excinfo.value.status_code

    async def test_fail_to_authorize_request_due_to_bearer_token_is_invalid(self, empty_request, jwt_auth):
    ...
    async def test_fail_to_authorize_request_due_to_bearer_token_is_missing(self, empty_request, jwt_auth):
    ...
    async def test_fail_to_authorize_request_due_to_blacklisted_token(self, mocker, cache_service, jwt_auth, jwt_token,
                                                                      valid_request):
        mocker.patch(
    ...
    async def test_successfully_authorize_request(self, mocker, cache_service, jwt_auth, jwt_token, valid_request):
